...
(defvar *krb-errors*
  '((:none 0 "No error")
    ...
    (:MUT-FAIL                   46  "Mutual authentication failed")
...
(define-condition kerberos-error (error)
  ((stat :initarg :stat :reader kerberos-error-stat)
   (desc :initarg :desc :initform nil :reader kerberos-error-desc))
  (:report (lambda (c stream)
	     (format stream "KERBEROS-ERROR ~A: ~A" 
		     (kerberos-error-stat c)
		     (or (kerberos-error-desc c)
			 (third (assoc (kerberos-error-stat c) *krb-errors*)))))))
